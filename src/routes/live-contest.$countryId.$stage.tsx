...
export const Route = createFileRoute("/live-contest/$countryId/$stage")({
      ...
      <section className="px-6 lg:px-16 py-12 grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {inStage.length === 0 && (
          <div className="col-span-full text-center py-16 text-muted-foreground">
            ...
            <Link to="/live-contest" className="text-accent">View all battles →</Link>
          </div>
        )}
        {inStage.map(c => (
          <Link key={c.id} to="/contestants/$slug" params={{ slug: c.slug }} className="group">
