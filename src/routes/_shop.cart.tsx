...
export function ShopCart() {
  ...
  const getParsedAddresses = () => {
    ...
    return rawList.map((addrStr: any) => {
      if (!addrStr) {
        return { name: user ? `${user.firstName} ${user.lastName}` : "Customer Name", address: "", phone: user?.phone || "" };
      }
      if (typeof addrStr === "object") {
        ...
        return {
          name: addrStr.full_name || addrStr.name || (user ? `${user.firstName} ${user.lastName}` : "Customer Name"),
      ...
      try {
        if (typeof addrStr === "string" && addrStr.trim().startsWith("{")) {
          ...
          const fullAddrStr = [street, city, stateVal, zip, country].filter(Boolean).join(", ");
          return {
            name: parsed.full_name || parsed.name || (user ? `${user.firstName} ${user.lastName}` : "Customer Name"),
