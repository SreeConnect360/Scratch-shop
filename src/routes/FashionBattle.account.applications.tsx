...
export const Route = createFileRoute("/FashionBattle/account/applications")({
      ...
      <FadeUp>
        <section>
          ...
          {state.drafts.length === 0 ? (
            <p className="mt-4 text-sm text-muted-foreground">No drafts yet. Start your <Link to="/FashionBattle/apply" className="underline">application</Link>.</p>
          ) : (
            <div className="mt-6 divide-y divide-border-subtle border-y border-border-subtle">
              {state.drafts.map(d => (
                <div key={d.id} className="py-4 flex items-center justify-between">
                  ...
                  <Link to="/FashionBattle/apply" className="editorial-label hover:text-accent">Continue →</Link>
                </div>
              ))}
            </div>
          )}
