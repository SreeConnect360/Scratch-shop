...
const BASE_NAV = [
  { to: "/FashionBattle/account", label: "Dashboard", icon: LayoutGrid, exact: true },
  { to: "/FashionBattle/account/profile", label: "Account Information", icon: User, exact: false },
  { to: "/FashionBattle/account/applications", label: "My Application Folder", icon: FileText, exact: false },
  { to: "/FashionBattle/account/notifications", label: "Notifications", icon: Bell, exact: false },
] as const;

const ROLE_NAV = [
  { role: "Applications", to: "/FashionBattle/account/role-applications", label: "Applications", icon: ClipboardList },
  { role: "Ratings", to: "/FashionBattle/account/role-ratings", label: "Ratings", icon: Star },
  { role: "Casting Call", to: "/FashionBattle/account/role-casting", label: "Casting Call", icon: UsersIcon },
  { role: "Judgements", to: "/FashionBattle/account/role-judgements", label: "Judgements", icon: Gavel },
] as const;
        ...
        <div className="min-h-[70vh] flex items-center justify-center px-6 text-center">
          <div>
            ...
            <div className="mt-8 flex gap-4 justify-center">
              <Link to="/FashionBattle/login" className="bg-foreground text-background px-8 py-3.5 editorial-label hover:bg-accent hover:text-white transition-colors">Sign In</Link>
              <Link to="/FashionBattle/register" className="border border-foreground px-8 py-3.5 editorial-label hover:bg-foreground hover:text-background transition-colors">Register</Link>
            </div>
          </div>
        </div>
      </PublicLayout>
        ...
        <aside className="lg:col-span-3 space-y-6">
          ...
          {(() => {
            const assignedRoles: { role: Role; label: string; to: string }[] = [
              { role: "Applications" as Role, label: "Applications", to: "/FashionBattle/account/role-applications" },
              { role: "Ratings" as Role, label: "Ratings", to: "/FashionBattle/account/role-ratings" },
              { role: "Casting Call" as Role, label: "Casting Call", to: "/FashionBattle/account/role-casting" },
              { role: "Judgements" as Role, label: "Judgement", to: "/FashionBattle/account/role-judgements" },
