...
function ShopRegisterPage() {
  ...
  const handleModalGoogleAuth = async (customEmail?: string) => {
    ...
    const firstName = names[0] || targetEmail.split("@")[0].split(".")[0];
  ...
  const googleLogin = useGoogleLogin({
    ...
    onError: (err) => {
      console.error("Google signup error, launching modal fallback:", err);
      setShowGoogleModal(true);
    },
  });
          ...
          {!success ? (
            <>
              ...
              <button
                ...
                className="w-full border border-white/10 bg-white/5 hover:bg-white/10 rounded-full py-3.5 text-xs font-bold uppercase tracking-widest flex items-center justify-center gap-3 transition-all cursor-pointer text-foreground active:scale-95"
              >
