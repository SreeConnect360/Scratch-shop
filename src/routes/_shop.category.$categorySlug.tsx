        ...
        {filteredProducts.length === 0 ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
            {filteredProducts.map((product) => {
              const inWishlist = state.user ? (state.shopWishlist[state.user.id] || []).includes(product.id) : false;
                ...
                >
                  ...
                  <div className="flex flex-1 flex-col p-4">
                    <span className="text-[10px] uppercase tracking-wider font-semibold text-gold mb-1">
                      {product.brand || product.house || "Maison Curation"}
                    </span>
                    <h3
                      onClick={() => navigate({ to: "/product/$productId", params: { productId: product.name.toLowerCase().replace(/[^a-z0-9]+/g, "-") } })}
                      ...
                      <button
                        onClick={() => {
                          ...
                          addToShopCart({
                            ...
                            name: product.name,
                            house: product.brand || product.house || "Maison Curation",
                            price: product.price,
