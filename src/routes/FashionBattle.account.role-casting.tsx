...
export const Route = createFileRoute("/FashionBattle/account/role-casting")({
  component: RoleCastingPage,
});

const TABS: WorkflowStatus[] = ["Approved", "Selected", "Hold", "Thank You", "Block"];
...
function RoleCastingPage() {
  const { state, setCastingStatus } = usePortal();
  ...
  const [tab, setTab] = useState<WorkflowStatus>("Approved");
  const [pendingCasting, setPendingCasting] = useState<Record<string, WorkflowStatus>>({});
  ...
  return (
    <div className="space-y-8">
      <header>
        <p className="editorial-eyebrow text-accent">Role · Casting Call</p>
        <h2 className="mt-2 font-serif text-3xl">Casting Decisions</h2>
      </header>

      <div className="flex flex-wrap gap-3">
        <select value={country} onChange={e => { setCountry(e.target.value); setPendingCasting({}); }} className="bg-transparent border border-border-subtle px-3 py-2 text-sm rounded bg-background">
      ...
      <div className="flex flex-wrap gap-1 editorial-label">
        {TABS.map(t => (
          <button key={t} onClick={() => { setTab(t); setPendingCasting({}); }} className={`px-3 py-1 border transition-colors ${tab === t ? "border-accent text-accent bg-accent/5" : "border-border-subtle text-muted-foreground hover:text-foreground"}`}>
            {t} ({apps.filter(a => (state.castingWorkflow[a.contestantId] ?? "Approved") === t).length})
          </button>
        ))}
      </div>
              ...
              <div key={a.contestantId} className="border border-border-subtle p-4 space-y-3 bg-surface rounded flex flex-col justify-between">
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <div className="editorial-label text-muted-foreground">{a.contestantId}</div>
                  </div>
                </div>
                ...
                <div className="space-y-2 mt-4 pt-2">
                  <select 
                    value={currentStatus} 
                    onChange={e => setPendingCasting(prev => ({ ...prev, [a.contestantId]: e.target.value as WorkflowStatus }))} 
