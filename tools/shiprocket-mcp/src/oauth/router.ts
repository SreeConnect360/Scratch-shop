...
oauthRouter.post(["/oauth/register", "/register"], async (req, res) => {
  const { redirect_uris, grant_types, token_endpoint_auth_method } = req.body as {
...
async function resolveRedirectUris(clientId: string): Promise<string[] | null> {
  ...
  if (registered) return registered.redirectUris;
  ...
  try {
    ...
    const { data } = await axios.get<{ redirect_uris?: string[] }>(clientId, {
...
oauthRouter.get(["/oauth/authorize", "/authorize"], async (req, res) => {
  ...
  if (!allowedUris.some((uri) => redirectUriMatches(uri, redirect_uri))) {
  ...
  res.send(
    loginPage({ client_id, redirect_uri, state, code_challenge, code_challenge_method, scope, resource })
  );
});
...
function loginPage(params: LoginPageParams, error?: string): string {
